"""Phase 2: Scrape content from discovered Redis URLs (loads URLs from file, skips Phase 1)."""
...
from scrapers.redis_scraper import RedisScraper
...
def get_latest_discovered_urls():
    ...
    url_files = glob.glob('data/scraped/redis/discovered_urls-*.json')
    if not url_files:
...
def main():
    ...
    print("=" * 70)
    print("PHASE 2: CONTENT SCRAPING (REDIS)")
    print("=" * 70)
    ...
    # Filter out the base customers page if present
    urls = [url for url in urls if url.rstrip('/') != 'https://redis.io/customers']
    print(f"📋 Filtered to {len(urls)} customer story URLs\n")
    ...
    existing_files = glob.glob('data/scraped/redis/*.json')
    ...
    for i, url in url_iterator:
        ...
        try:
            ...
            if ref_data:
                ...
                filepath = save_reference_file('Redis', ref_data)
                if filepath:
                    if not USE_TQDM:
    ...
    print(f"💾 Files saved: {len(references)} files in data/scraped/redis/")
    ...
    print(f"  Files saved in: data/scraped/redis/")
